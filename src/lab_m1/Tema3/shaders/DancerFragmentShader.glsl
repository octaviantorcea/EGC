...
// self properties
uniform float dancer_Kd;
uniform float dancer_Ks;
uniform int material_shininess;

// danceFloor light
uniform vec3 danceFloor_light_position[9];
uniform float floorTileLightRadius;
uniform vec3 danceFloor_light_color[9];
...
vec3 light_from_dance_floor_tile(vec3 lightPos, vec3 lightColor) {
    ...
    if (dist < floorTileLightRadius) {
        ...
        float diffuseLight = dancer_Kd * max(dot(N, L), 0);
        float specularLight = 0;

        if (dot(N, L) > 0) {
            specularLight = dancer_Ks * pow(max(dot(N, H), 0), material_shininess);
        }

...
// self properties
uniform vec3 floor_Ke;
uniform float floor_Kd;
uniform float floor_Ks;
...
vec3 light_from_spotlight(int index) {
    ...
    if (spot_light > spot_light_limit) {
        ...
        float diffuseLight = floor_Kd * max(dot(N, L), 0);
        float specularLight = 0;

        if (dot(N, L) > 0) {
            specularLight = floor_Ks * pow(max(dot(N, H), 0), shininess);
        }
...
vec3 light_from_globe() {
	...
	color = fAtt * globe_intensity * 0.25f * globe_color * (diffuseLight + specularLight);
